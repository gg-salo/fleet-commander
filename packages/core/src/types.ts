...
export interface Runtime {
  ...
  /**
   * Optional: send raw keystrokes (e.g. "Down", "Enter", "C-c") to the session.
   * Unlike sendMessage, these are NOT treated as literal text.
   * Used for navigating interactive TUI prompts after restore.
   */
...
export interface Agent {
  ...
  /**
   * Optional: get tmux keystrokes to send after a restore command launches.
   * Some agents show interactive confirmation dialogs on resume (e.g. Claude Code's
   * bypass-permissions prompt). Returns an array of [keys, delayMs] pairs to send
   * sequentially via tmux send-keys, or null if no post-restore interaction needed.
   */
  getPostRestoreKeys?(project: ProjectConfig): Array<{ keys: string; delay: number }> | null;
...
export interface SCM {
  ...
  /** List open PRs for a project (optional) */
...
/** Unique plan identifier, e.g. "plan-1740500000-abc123" */
...
export interface PlanTask {
  /** Task identifier within the plan (e.g. "1", "2") */
  id: string;
  ...
  /** Estimated scope */
  scope: "small" | "medium";
  /** IDs of tasks this depends on (must finish first) */
  dependencies: string[];
  /** GitHub issue number (set after approval) */
  issueNumber?: number;
  /** GitHub issue URL (set after approval) */
  issueUrl?: string;
  /** Coding agent session ID (set after approval) */
  sessionId?: string;
  /** Files this task will likely modify */
  affectedFiles?: string[];
  /** Architectural constraints the coding agent must follow */
  constraints?: string[];
  /** Context about other tasks in this plan touching the same areas */
  sharedContext?: string;
}

/** A feature plan — generated by the planning agent, reviewed by the user */
export interface Plan {
  ...
  /** Current lifecycle status */
  status: PlanStatus;
  /** Session ID of the planning agent */
...
/** Unique discovery identifier, e.g. "discovery-1740500000-abc123" */
export type DiscoveryId = string;

/** Discovery agent types */
export type DiscoveryType = "ux-audit" | "competitor-research" | "code-health";

/** Discovery lifecycle states */
export type DiscoveryStatus = "discovering" | "ready" | "failed";

/** A single finding from a discovery agent */
...
/** A discovery — generated by a discovery agent, reviewed by the user */
...
/** Unique reconciliation identifier, e.g. "reconciliation-1740500000-abc123" */
export type ReconciliationId = string;

/** Reconciliation lifecycle states */
export type ReconciliationStatus = "analyzing" | "ready" | "failed";

/** A single finding from the reconciliation agent */
export interface ReconciliationFinding {
  id: string;
  type: "duplication" | "conflict" | "inconsistency" | "merge-order";
  title: string;
  description: string;
  affectedPRs: number[];
...
/** A reconciliation — cross-PR consistency check after all plan tasks complete */
...
/** Unique review batch identifier, e.g. "review-batch-1740500000-abc123" */
export type ReviewBatchId = string;

/** Per-PR status within a review batch */
...
/** A single PR within a review batch */
...
export type ReviewBatchStatus = "reviewing" | "fixing" | "done" | "failed";

/** A batch of PRs being reviewed together */
